...
def estimate_isoform_expression_grid_search_iteration(SR_isoform_region_matrix,SR_region_read_count_matrix,LR_isoform_region_matrix,LR_region_read_count_matrix,isoform_lengths,alpha,beta,P):
    num_isoforms = SR_isoform_region_matrix.shape[1]
    Q = 2 * (1.0 - alpha) * np.matmul(SR_isoform_region_matrix.T,SR_isoform_region_matrix) + 2 * alpha * np.matmul(LR_isoform_region_matrix.T,LR_isoform_region_matrix) + 2 * beta * np.identity(num_isoforms)
    c = -2 * (1.0 - alpha) * np.matmul(SR_isoform_region_matrix.T,SR_region_read_count_matrix.T) - 2 * alpha * np.matmul(LR_isoform_region_matrix.T,LR_region_read_count_matrix.T)
    lb = np.zeros(num_isoforms)
...
def estimate_isoform_expression(SR_isoform_region_matrix,SR_region_read_count_matrix,LR_isoform_region_matrix,LR_region_read_count_matrix,isoform_lengths,alpha,beta,P):
    ...
    future_grid_search = {executor.submit(estimate_isoform_expression_grid_search_iteration, SR_isoform_region_matrix,SR_region_read_count_matrix,LR_isoform_region_matrix,LR_region_read_count_matrix,isoform_lengths,params['alpha'],params['beta'],P): params for params in params_grid}
    ...
    print('Best Params is {}'.format(min_target))
    return best_isoform_expression 

                
def TransELS(ref_file_path,short_read_alignment_file_path,long_read_alignment_file_path,output_path,region_expression_calculation_method,alpha,beta,P,READ_LEN=79,READ_JUNC_MIN_MAP_LEN=10):
    ...
    long_read_gene_regions_read_length,total_long_read_length,num_LRs = parse_alignment(long_read_alignment_file_path,READ_LEN,READ_JUNC_MIN_MAP_LEN,gene_regions_points_list,gene_range,gene_regions_dict, long_read = True)
    ...
    long_read_gene_matrix_dict = generate_all_feature_matrix_long_read(gene_isoforms_dict,gene_regions_dict,long_read_gene_regions_read_length,genes_regions_len_dict,total_long_read_length,region_expression_calculation_method)
            ...
            for isoform_name in isoform_names_indics:
                ...
                try:
                    gene_isoform_expression_dict[chr_name][gene_name] = estimate_isoform_expression(
                                            ...
                                            long_read_gene_matrix_dict[chr_name][gene_name]['region_read_count_matrix'],isoform_lengths,alpha,beta,P)
