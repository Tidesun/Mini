...
def EM_worker(worker_id,output_path,eff_len_arr,num_SRs,num_LRs):
    ...
    for community_ANT,community_cond_prob,community_isoform in zip(worker_file['ANT'],worker_file['cond_prob'],worker_file['isoform']):
...
def EM_manager(isoform_gene_dict,isoform_index_dict,eff_len_arr,output_df,output_path,threads):
    ...
    for worker_id in range(threads):
        futures.append(pool.apply_async(EM_worker,(worker_id,output_path,eff_len_arr,num_SRs,num_LRs,),error_callback=callback_error))
    all_LR_TPM_df = []
    all_SR_TPM_df = []
    for future in futures:
